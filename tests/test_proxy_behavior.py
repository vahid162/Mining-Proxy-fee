...
def test_proxy_fee_route_starts_disarmed() -> None:
    session = MinerSessionState(session_id="s00000001", client_addr="127.0.0.1")

    assert session.fee_ready is False
    assert session.fee_needs_fresh_notify is True


def test_proxy_fee_route_disarms_on_state_boundaries() -> None:
    proxy = MinerProxy(Settings(fee_user="fee.wallet.worker"), MinerMetrics())
    session = MinerSessionState(
        session_id="s00000001",
        client_addr="127.0.0.1",
        fee_authorized=True,
        fee_ready=True,
        fee_needs_fresh_notify=False,
    )

    proxy._disarm_fee_route(session, "fee_set_difficulty")
    assert session.fee_ready is False
    assert session.fee_needs_fresh_notify is True

    session.fee_ready = True
    session.fee_needs_fresh_notify = False
    proxy._disarm_fee_route(session, "fee_set_extranonce")
    assert session.fee_ready is False
    assert session.fee_needs_fresh_notify is True

    session.fee_ready = True
    session.fee_needs_fresh_notify = False
    proxy._disarm_fee_route(session, "upstream_reconnect")
    assert session.fee_ready is False
    assert session.fee_needs_fresh_notify is True


def test_proxy_fee_route_arms_only_when_authorized_and_transition_free() -> None:
    proxy = MinerProxy(Settings(fee_user="fee.wallet.worker"), MinerMetrics())
    session = MinerSessionState(session_id="s00000001", client_addr="127.0.0.1")

    assert proxy._can_arm_fee_route(session) is False

    session.fee_authorized = True
    session.awaiting_post_difficulty_job = True
    assert proxy._can_arm_fee_route(session) is False
    ...
    assert proxy._can_arm_fee_route(session) is True

    proxy._arm_fee_route(session, "job-fee")
    assert session.fee_ready is True
    assert session.fee_needs_fresh_notify is False


def test_proxy_fee_route_selection_falls_back_to_main_until_ready() -> None:
    async def _run() -> None:
        metrics = MinerMetrics()
        proxy = MinerProxy(Settings(fee_user="fee.wallet.worker", fee_ratio=1.0), metrics)
        controller = FeeController(1.0, 5)
        ...
        session = MinerSessionState(
            ...
            fee_authorized=True,
            fee_ready=False,
            fee_needs_fresh_notify=True,
        )

        route = await proxy._select_notify_route(session, controller, tracker, "job-1")
